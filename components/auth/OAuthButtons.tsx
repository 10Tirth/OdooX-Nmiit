  ...
  return (
    <div className="space-y-3">
      ...
      <button
        ...
        aria-label={`Continue with Google to ${mode === 'signin' ? 'sign in' : 'sign up'}`}
      >
        {loading === 'google' ? (
      ...
      {/* Apple OAuth - only show on supported devices */}
      {isAppleSupported() && (
        <button
          onClick={() => handleOAuth('apple')}
          disabled={loading === 'apple'}
          className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-lg bg-black text-white hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          aria-label={`Continue with Apple to ${mode === 'signin' ? 'sign in' : 'sign up'}`}
        >
          {loading === 'apple' ? (
      ...
      <p className="text-xs text-gray-500 text-center">
        We only use Google/Apple to verify your identity — we never post to your account.
      </p>
    </div>
  );
}
